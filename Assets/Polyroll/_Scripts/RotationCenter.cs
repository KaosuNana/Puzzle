...
public class RotationCenter : MonoBehaviour {
	...
	void ManageControl()
	{
		...
		else if (Input.GetMouseButton(0))
		{
			...
			Debug.Log("Mouse:: TargetX: " +targetRotX);
			Debug.Log("Mouse:: TargetY: " +targetRotY);
			...
			// Debug.Log("Mouse:: TargetX: " +targetRotX);
			// Debug.Log("Mouse:: TargetY: " +targetRotY);
			// Debug.Log("Mouse:: transform.rotation.x: " +transform.rotation.x);
			// Debug.Log("Mouse:: Quaternion.identity.x: " +Quaternion.identity.x);
		}
		else
		{
			...
			{
				float dx = -1 / (float)Screen.width * -rotationSpeed;
				canRotate = true;
				targetRotX = Mathf.Lerp(targetRotX, dx, smooth) * Time.deltaTime;
				transform.RotateAround(Vector3.zero, Vector3.up, targetRotX);
			}		
			...
			{
				float dy = 1 / (float)Screen.height * rotationSpeed;
				canRotate = true;
				// targetRotY = Mathf.Lerp(targetRotX, 20, smooth) * Time.deltaTime;
				targetRotY = Mathf.Lerp(targetRotX, dy, smooth) * Time.deltaTime;
				transform.RotateAround(Vector3.zero, Vector3.right, targetRotY);
			}		
			...
			if(Mathf.Abs(transform.rotation.x - Quaternion.identity.x) < 0.08f &&
			   Mathf.Abs(transform.rotation.y - Quaternion.identity.y) < 0.08f)
			{
				canRotate = false;
			}
			// Debug.Log("Key:: transform.rotation.x: " +transform.rotation.x);
			// Debug.Log("Key:: Quaternion.identity.x: " +Quaternion.identity.x);
			if (!TipSprite.gameObject.activeSelf)
			{
				if (Input.GetButtonDown("Submit"))
				{
					if (PlayerPrefs.GetInt("Coin") >= 400)
					{
						...
						TipSprite.sprite = imageSR.sprite;
